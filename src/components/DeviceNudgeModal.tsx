...
export default function DeviceNudgeModal({ result, onClose }: DeviceNudgeModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div
        className="relative w-full max-w-4xl bg-[#161B22] border border-[#2D3748] rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Left: Device Simulator */}
        <div className="md:w-1/2 bg-[#0A0D14] p-6 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-[#1E2533]">
          {/* Channel selector tabs */}
          <div className="flex gap-2 p-1 bg-[#161B22] rounded-xl border border-[#2A3244] mb-4 w-full max-w-[280px]">
            <button
          ...
          {/* Smartphone Frame */}
          <div className="w-[300px] h-[520px] bg-[#12161F] rounded-[40px] border-[6px] border-[#2A3244] shadow-2xl relative overflow-hidden flex flex-col">
            {/* Notch / Dynamic Island */}
            <div className="h-6 w-full flex justify-between items-center px-6 pt-2 text-[10px] text-[#8B949E]">
              <span>9:41</span>
            ...
            {activeChannel === "whatsapp" && (
              <div className="flex-1 flex flex-col bg-[#0B141B] text-[#E9EDEF] overflow-y-auto">
                ...
                <div className="bg-[#1F2C34] px-3 py-2 flex items-center gap-2.5 border-b border-[#2A3942]">
                  <div className="w-8 h-8 rounded-full bg-[#00A884] flex items-center justify-center font-bold text-white text-xs">
                    R
                  </div>
                ...
                <div className="flex-1 p-3 space-y-2 text-xs">
                  <div className="bg-[#1F2C34] rounded-lg p-2.5 max-w-[90%] shadow text-[11px] leading-relaxed border-l-4 border-[#00A884]">
            ...
            {activeChannel === "push" && (
              <div className="flex-1 flex flex-col bg-gradient-to-b from-[#1C1C1E] to-[#0A0A0C] p-4 text-white">
                <div className="text-center text-[10px] text-[#8E8E93] mt-8 mb-4">NOTIFICATION BANNER</div>
        ...
        <div className="md:w-1/2 p-6 flex flex-col overflow-y-auto">
          <div className="flex justify-between items-start mb-4">
            <div>
              <span className="text-[10px] font-bold text-[#388BFD] uppercase tracking-wider bg-[#388BFD]/10 px-2 py-0.5 rounded border border-[#388BFD]/20">
                Agent 3: Behavioral Communication Output
              </span>
              <h3 className="text-lg font-bold text-[#F0F6FC] mt-1">Customer Recovery Experience</h3>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="space-y-4 text-xs">
            <div className="bg-[#1C2333] p-4 rounded-xl border border-[#2D3748]">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[#1C2333] p-3 rounded-lg border border-[#2D3748]">
              ...
              <div className="bg-[#1C2333] p-3 rounded-lg border border-[#2D3748]">
                <span className="text-[#8B949E] block text-[10px]">AI Estimated LTV Saved</span>
            ...
            <div className="bg-[#0E1117] p-4 rounded-xl border border-[#2D3748]">
          ...
          <div className="mt-auto pt-4 flex justify-end">
            <button
              onClick={onClose}
              className="bg-[#2D3748] hover:bg-[#3D4758] text-white px-5 py-2 rounded-xl font-semibold text-xs transition-colors cursor-pointer"
            >
              Close Simulator
            </button>
          </div>
